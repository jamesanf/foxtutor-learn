...
function accountingBillingSettingsPage(
  ...
  const connectionDetails = `<dl class="accounting-connection-details"><div><dt>Connection status</dt><dd>${integrationTag}</dd></div><div><dt>Environment</dt><dd>${escapeHtml(status.environment)}</dd></div><div><dt>Company name</dt><dd>${escapeHtml(status.companyName ?? "Not recorded — reauthenticate to refresh")}</dd></div><div><dt>Company subdomain</dt><dd>${escapeHtml(status.companySubdomain ?? "Not configured")}</dd></div>${status.updatedAt ? `<div><dt>Connection updated</dt><dd>${escapeHtml(notificationTimestamp(status.updatedAt))}</dd></div>` : ""}</dl>`;
  const warning = error
  ...
  const categoryOptions = categories.length
    ? `<option value="">Select a FreeAgent accounting category</option>${categories.map((category) => `<option value="${escapeHtml(category.url)}"${category.url === values.categoryUrl ? " selected" : ""}>${escapeHtml(category.name)}${category.nominalCode ? ` (${escapeHtml(category.nominalCode)})` : ""}</option>`).join("")}`
  ...
  if (route === "admin-accounting-settings") {
    ...
    if (status.connected && environment) {
      try {
        ...
        categoryError = error instanceof FreeAgentApiError
          ? `FreeAgent categories could not be loaded (${error.shape.code}).`
          : "FreeAgent categories could not be loaded.";
      }
