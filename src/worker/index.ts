...
function lessonReportForm(
  ...
  const editor = (label: string, name: string, content: string, required = false, toolbarLabel = label || "Notes"): string => `<div class="report-editor"><label>${label ? escapeHtml(label) : ""}<textarea name="${name}" rows="5" maxlength="12000"${required ? " required" : ""}>${escapeHtml(content)}</textarea></label><div class="report-toolbar" role="toolbar" aria-label="${escapeHtml(toolbarLabel)} formatting"><button type="button" class="report-tool" data-report-format="bullet" title="Bullet points"><svg class="report-tool-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 5v2H5V5h2Zm4 0v2h10V5H11ZM7 11v2H5v-2h2Zm4 0v2h10v-2H11ZM7 17v2H5v-2h2Zm4 0v2h10v-2H11Z"/></svg></button><button type="button" class="report-tool" data-report-format="bold" title="Bold"><strong>B</strong></button><button type="button" class="report-tool report-tool-highlight" data-report-format="highlight" title="Yellow highlight">A</button></div></div>`;
...
function reportTime(lesson: Lesson): string {
  ...
  const format = (value: string) => formatter.format(new Date(value)).replace(" ", "").toUpperCase().replace(/:00(AM|PM)$/, "$1");
  return `${format(lesson.start_at)}–${format(lesson.end_at)}`;
}
