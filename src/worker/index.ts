...
function lessonReportForm(
  ...
  const editor = (label: string, name: string, content: string, required = false, toolbarLabel = label || "Notes"): string => `<div class="report-editor" data-report-editor data-list-mode="bullet"><label>${label ? escapeHtml(label) : ""}<textarea name="${name}" rows="5" maxlength="12000"${required ? " required" : ""}>${escapeHtml(content)}</textarea></label><div class="report-toolbar" role="toolbar" aria-label="${escapeHtml(toolbarLabel)} formatting"><button type="button" class="report-tool report-list-mode is-active" data-report-format="list-bullet" aria-pressed="true" title="Bullet points"><svg class="report-tool-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 5v2H5V5h2Zm4 0v2h10V5H11ZM7 11v2H5v-2h2Zm4 0v2h10v-2H11ZM7 17v2H5v-2h2Zm4 0v2h10v-2H11Z"/></svg></button><button type="button" class="report-tool report-list-mode" data-report-format="list-numbered" aria-pressed="false" title="Numbered list">1.</button><button type="button" class="report-tool" data-report-format="bold" title="Bold"><strong>B</strong></button><button type="button" class="report-tool report-tool-highlight" data-report-format="highlight" title="Yellow highlight">A</button><button type="button" class="report-tool report-list-mode" data-report-format="list-off" aria-pressed="false" title="Turn lists off">×</button></div></div>`;
  const notes = value("notes") || value("additional_notes");
  return `<section class="card form-card report-form"><h1>Lesson Report</h1>${error ? `<p class="form-error" role="alert">${escapeHtml(error)}</p>` : ""}<form method="post" action="${action}">${hiddenCsrf(csrfToken)}<div class="report-meta"><p><strong>Date</strong><br>${escapeHtml(reportDate(lesson))}</p><p><strong>Pupil</strong><br>${escapeHtml(pupil)}</p><p><strong>Time</strong><br>${escapeHtml(reportTime(lesson))}</p>${levelControl}</div><div class="report-feedback-form">${editor("This Lesson's Focus", "thisLessonsFocus", value("this_lessons_focus") || value("summary"), true)}${editor("Next Lesson's Focus", "nextLessonsFocus", value("next_lessons_focus"))}${editor("Even Better If", "evenBetterIf", value("even_better_if"))}${editor("Home Learning Task", "homeLearningTask", value("home_learning_task") || value("homework"))}<details class="report-notes-details"${notes ? " open" : ""}><summary>Notes</summary>${editor("", "notes", notes, false, "Notes")}</details></div><div class="form-actions"><a class="button secondary" href="/learn/admin/lessons/${encodeURIComponent(lesson.id)}">Cancel</a><button class="button secondary" type="submit" name="action" value="save">Save draft</button><button class="button" type="submit" name="action" value="send">Send report</button></div></form>${reportAttachmentUploadForm(csrfToken, lesson, student)}</section>`;
}

function reportAttachmentUploadForm(csrfToken: string, lesson: Lesson, student: Student): string {
  const id = `report-attachment-${lesson.id.replace(/[^a-zA-Z0-9_-]/g, "")}`;
  return `<section class="report-attachments"><div class="section-heading"><div><h2>Lesson attachments</h2><p class="muted">Add slides, homework or other files for this lesson.</p></div></div><form class="resource-upload-form report-attachment-form" method="post" action="/learn/admin/resources/new" enctype="multipart/form-data" data-return-context="lesson">${hiddenCsrf(csrfToken)}<input type="hidden" name="idempotencyKey" value="${escapeHtml(crypto.randomUUID())}"><input type="hidden" name="returnContext" value="lesson"><input type="hidden" name="studentId" value="${escapeHtml(student.id)}"><input type="hidden" name="lessonId" value="${escapeHtml(lesson.id)}"><div class="resource-file-dropzone" data-file-dropzone tabindex="0" role="button" aria-labelledby="${id}-label" aria-describedby="${id}-help"><span class="resource-file-icon" aria-hidden="true">↥</span><strong id="${id}-label">Drop a file here or <span class="resource-browse">Browse</span></strong><span id="${id}-help" class="field-help">PDF · DOCX · TXT · PNG · JPEG · WEBP · Up to 25 MB</span><input id="${id}" type="file" name="file" aria-label="Choose lesson attachment" required accept=".pdf,.docx,.txt,.png,.jpg,.jpeg,.webp"><output class="file-preview" data-file-preview aria-live="polite"></output></div><p class="upload-status" data-upload-status aria-live="polite"></p><div class="form-actions"><button class="button" type="submit">Upload attachment</button></div></form></section>`;
}
...
function lessonTable(lessons: Lesson[], basePath: string, showStudent: boolean): string {
  ...
  return `<div class="table-wrap"><table><thead><tr>${showStudent ? "<th>Student</th>" : "<th>Lesson</th>"}<th>Date and time</th><th>Status</th><th>Report</th></tr></thead><tbody>${lessons.map((lesson) => lessonRow(lesson, basePath, showStudent)).join("")}</tbody></table></div>`;
}
