...
function accountingList(
  ...
  const connectionCard = (environment: FreeAgentEnvironment): string => {
    ...
    const connectionAction = buttonLink(
      ...
      status.connected ? `Reauthenticate ${label}` : `Connect ${label}`
    );
...
function accountingBillingSettingsPage(
  ...
  const connectionAction = `<a class="button secondary" href="/learn/admin/accounting/connect/${environment}">${status.connected ? `Reauthenticate ${environmentLabel}` : `Connect ${environmentLabel}`}</a>`;
