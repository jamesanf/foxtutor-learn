...
function recurringSeriesSection(
  ...
  return `<section class="recurring-series-section"><div class="section-heading"><div><h2>Recurring lesson series</h2><p class="muted">Future lessons are materialised through the bounded six-week Europe/London horizon.</p></div>${buttonLink("/learn/admin/series/new", "Create series")}</div><div class="table-wrap"><table><thead><tr><th>Student</th><th>Weekly time</th><th>Duration</th><th>Price</th><th>Dates</th><th>Status</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table></div>${studentSectionPagination(page, pageSize, total, "/learn/admin/bookings", "Recurring lesson series", "seriesPage", "seriesSize")}</section>`;
}
...
function recurringSeriesFormMarkup(csrfToken: string, students: Student[], error?: string, dialog = false): string {
  ...
  const cancel = dialog
    ? `<button class="button secondary" type="button" data-booking-choice-back>Back</button>`
