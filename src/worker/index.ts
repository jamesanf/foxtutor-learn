...
function navigation(role: Role): string {
  const icons = {
    ...
    accounting: "M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm1 16h-2v-2h2v2Zm2.2-6.75-.9.92c-.72.73-1.3 1.33-1.3 2.83h-2c0-2.08.9-3 1.88-3.98l.9-.92c.48-.48.72-.98.72-1.6 0-1.1-.9-2-2-2s-2 .9-2 2h-2a4 4 0 1 1 6.7 2.75Z",
...
function accountingBillingSettingsPage(
  ...
  return `<section class="card form-card"><div class="page-heading"><div><h1>Billing settings</h1><p class="lede">Configure the values used for future FreeAgent lesson invoices.</p></div><a class="button secondary" href="/learn/admin/accounting">Back to accounting</a></div>${error ? `<p class="form-error" role="alert">${escapeHtml(error)}</p>` : ""}<p class="muted">GBP is enforced for every invoice. The starting lesson amount is £55.00, item type is Hours, payment terms are 0 days, and no VAT is charged. These settings can be changed here, but unsupported or incomplete values fail closed.</p><p class="info-box"><strong>FreeAgent setup:</strong> category URL and contact mappings must come from the connected FreeAgent company. OAuth client secrets and the token encryption key cannot be entered here; configure them as Cloudflare Worker secrets through the dashboard or <code>wrangler secret put</code>. Secret values are never shown or stored in this form.</p><form method="post" action="/learn/admin/accounting/settings"><input type="hidden" name="csrf" value="${escapeHtml(csrfToken)}"><div class="lesson-form-grid"><label>Lesson amount<input name="amount" inputmode="decimal" pattern="\\d+(\\.\\d{1,2})?" value="${escapeHtml(values.amount)}" required><span class="field-help">Use pounds and pence, for example 55.00.</span></label><label>Currency<input name="currency" value="GBP" readonly aria-readonly="true"><span class="field-help">GBP is fixed by the accounting contract.</span></label><label>FreeAgent item type<input name="itemType" maxlength="240" value="${escapeHtml(values.itemType)}" required><span class="field-help">Hours is the default for lesson invoices; change it only if the connected company uses another supported item type.</span></label><label>FreeAgent category URL<input name="categoryUrl" type="url" value="${escapeHtml(values.categoryUrl)}" required><span class="field-help">Copy the category URL from the connected FreeAgent company. It is intentionally not invented here.</span></label><label>Payment terms (days)<input name="paymentTermsDays" type="number" min="0" max="365" step="1" value="${escapeHtml(values.paymentTermsDays)}" required></label><label>Sales tax rate<input name="salesTaxRate" inputmode="decimal" pattern="\\d+(\\.\\d{1,2})?" value="${escapeHtml(values.salesTaxRate)}" required><span class="field-help">0 means no VAT charged. An explicit rate is always sent to FreeAgent.</span></label></div><div class="form-actions"><a class="button secondary" href="/learn/admin/accounting">Cancel</a><button class="button" type="submit">Save billing settings</button></div></form></section>`;
}
...
async function adminDashboard(user: AppUser, csrfToken: string, db: D1Database): Promise<Response> {
  ...
  return appPage(user, csrfToken, "Dashboard", `<div class="page-heading"><h1>Dashboard</h1>${buttonLink("/learn/admin/lessons/new", "Add lesson")}</div><div class="summary-grid"><a class="summary-card" href="${nextLessonHref}"><span>Next Lesson</span><strong>${upcoming[0] ? escapeHtml(bookingDate(upcoming[0])) : "None"}</strong>${upcoming[0] ? `<small>${escapeHtml(bookingTime(upcoming[0]))}</small>` : ""}</a><a class="summary-card" href="/learn/admin/bookings"><span>Upcoming Bookings</span><strong>${upcomingCount}</strong><small>View bookings</small></a><a class="summary-card" href="/learn/admin/students"><span>Active Students</span><strong>${activeStudents}</strong><small>View students</small></a><a class="summary-card" href="/learn/admin/reschedules"><span>Reschedule requests</span><strong>${rescheduleRequests}</strong><small>${rescheduleRequests ? "Review requests" : "View reschedules"}</small></a></div><section class="card dashboard-section"><div class="section-heading"><h2>Reports to write</h2><a class="text-link" href="/learn/admin/lessons">Past Lessons</a></div>${reportPreview}</section><section class="card dashboard-section"><div class="section-heading"><h2>Upcoming Bookings</h2><a class="text-link" href="/learn/admin/bookings">See all</a></div>${preview}</section>`);
}
