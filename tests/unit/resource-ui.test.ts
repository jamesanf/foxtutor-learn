...
describe("resource UX contract", () => {
  ...
  it("groups report actions and tracks saved drafts until the form changes", () => {
    ...
    expect(clientSource).toContain('const formAction = form.getAttribute("action")');
    expect(clientSource).not.toContain('new URL(form.action, window.location.href)');
  ...
  it("uses a minimal, non-duplicated product header", () => {
    ...
    expect(cssSource).toContain(".app-shell { display: flex; min-height: 100vh; flex-direction: column; }");
    expect(cssSource).toContain(".layout { display: grid; grid-template-columns: 220px minmax(0, 1fr); min-height: 0; flex: 1 0 auto; }");
