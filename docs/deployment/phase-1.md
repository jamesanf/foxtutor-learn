...
1. Use the existing Wrangler OAuth session for Worker deployment, D1 and Workers Routes operations. Before any production mutation, obtain or expose an existing credential with Zero Trust Access application, identity-provider and policy write permissions; do not create a replacement credential if an authorized existing mechanism is available.
...
Phase 1.3 tested both the environment API token and the existing Wrangler OAuth session through Wrangler and direct Cloudflare REST. The OAuth session can read and provisionally authorize Worker, D1 and Workers Routes operations, and can read R2; the environment token cannot. Neither existing mechanism can write Zero Trust Access applications or identity providers. The account inventory and exact sanitized results are in `docs/evidence/phase-1/cloudflare-capability-check.txt`.
