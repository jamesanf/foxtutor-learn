...
1. Configure the sandbox FreeAgent environment, client ID, client secret,
   ...
   approved invoice mapping values.
2. Confirm the configured environment is `sandbox` and the callback exactly
   matches the registered URI.
3. As an admin, open `/learn/admin/accounting` and start the FreeAgent
   connection.
4. Complete OAuth, verify the returned company subdomain and confirm the
   connection status.
5. For the approved test payer, enter the numeric FreeAgent contact ID.
   Confirm the server verifies the contact in the pinned company before saving.
6. Create or select the approved `ADMIN_CANCELLED` test event. Confirm one
   outbox row, the deterministic accounting reference and the visible blocked
   state before retry.
7. Run the event through the scheduled processor or the approved operational
   trigger. Verify the single draft invoice, contact, date, currency, item,
   category, tax and amount in FreeAgent.
8. Replay the worker and confirm no second invoice is created.
9. Exercise the approved timeout/unknown test seam, reconcile the existing
   provider invoice and confirm the external reference is stored.
10. Exercise a retryable provider result and confirm bounded backoff, then
    inspect the admin retry audit record.
11. Verify that replacing or removing a mapping is blocked while dependent
