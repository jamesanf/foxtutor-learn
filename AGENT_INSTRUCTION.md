...
## Phase 2.8 current state

Phase 2.8 exists because real browser review of the Phase 2.7 implementation still found an oversized calendar, weak Previous/Next affordances, verbose subscription content, browser-native regeneration confirmation and unnecessary lesson-creation complexity. Keep the calendar compact, Month as the default and Week as the only secondary view. Previous/Next must use the coherent FullCalendar chevron iconography with accessible period-aware labels. The subscription utility remains below the calendar, collapsed by default, concise and secondary; browser-native `alert`, `confirm` and `prompt` dialogs are prohibited in Learn workflows. Standard lesson creation is 55 minutes, with UK/`Europe/London` as the normal visible timezone context and 15-minute start increments; Notes must not dominate creation and remain supported by the domain/edit workflow. Preserve UTC instant storage, IANA validation, overlap protection, authorization, ownership, feed/token, Access and D1 architecture.

README must be updated after every material UX, testing, deployment or cleanup pass. Documentation must be read before every phase/pass. Phase 2 cannot close until actual desktop, 75% zoom, tablet and mobile browser acceptance, accessibility checks, production acceptance, feed/Access/privacy regression and cleanup are evidenced. Do not create `phase-2.8-complete` or any final Phase 2 tag early.
