-- Student cancellation undo and late reschedule requests.
PRAGMA foreign_keys = OFF;

ALTER TABLE lesson_history RENAME TO lesson_history_phase5;
...
SELECT * FROM lesson_history_phase5;

DROP TABLE lesson_history_phase5;

CREATE INDEX idx_lesson_history_lesson_created
  ON lesson_history(lesson_id, created_at DESC);
CREATE INDEX idx_lesson_history_student_created
  ON lesson_history(student_id, created_at DESC);
CREATE UNIQUE INDEX idx_lesson_history_request_event
...
CREATE INDEX IF NOT EXISTS idx_reschedule_requests_status_created
  ON lesson_reschedule_requests(status, created_at);
CREATE INDEX IF NOT EXISTS idx_reschedule_requests_student_created
  ON lesson_reschedule_requests(student_id, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS idx_one_pending_reschedule_request
  ON lesson_reschedule_requests(lesson_id)
